...
function resolverTarefa() {
  const tarefa = lista.removeElemento();
  if (tarefa != null) {
...
function mostrarTarefaMaisAntiga() {
  ...
  if (!lista.isEmpty()) {
    let tarefaMaisAntiga = lista.head.dado;
...
function atualizarLista() {
  ...
  if (!lista.isEmpty()) {
    for (const tarefa of lista) {
      ...
      novaLinha.innerHTML = `${tarefa.toString()} <button onclick="removerTarefa(${tarefa._id})">Remover</button>`;
